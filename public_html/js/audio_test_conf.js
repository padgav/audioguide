...
var extMsg;
$.getJSON(msg_conf_file, function(json) {
    //console.log(json); // this will show the info in firebug console
    msg_conf = json;
    console.log("msg_conf inner (messaggi): ",msg_conf);
    // console.log("msg_conf welcome[0]: ",msg_conf["welcome"][0]);
});
$.getJSON(adorazione_conf_path, function(json) {
    console.log("the curr pic json: ", json); // this will show the info in firebug console
    current_pic = json;
    console.log("currentPic inner (adorazione): ",current_pic);
    //console.log("currentPic currentPic: ",currentPic);
});


/*
var request = new XMLHttpRequest();
request.open("GET", current_pic_file, true);
request.send(null)
var my_JSON_object = JSON.parse(request.responseText);
current_pic = my_JSON_object;
...
$(document).ready(function() {
  ///subjects keys: 37 38 39 40 65 68 70 83 87


  console.log("currentPic onReady (adorazione): ",current_pic);
  console.log("msg_conf onReady (messaggi): ",msg_conf);

  //console.log("msg_conf welcome[0]: ",msg_conf["welcome"][0]);
  //console.log("msg_conf welcome[0] on ready: ",msg_conf["welcome"][0]);

  var myMsg = {
      welcome: {  1: messages['welcome']/*msg_conf['welcome']*//*+msg_conf["welcome"][1]*/, 2:messages['welcome2'], 3:messages['welcome3'] },
  ...
  //tessera 0008657628 key 0000142480 // tessera su portatile 0008631935 (adorazione)
  var myPaints = {
        1: {name: "Adorazione", key: "0000142480", src: "./images/adorazione_pic.jpg"},
...
*/
        ...
        function Tasto(e) {
          ...
          if($(".card").attr("id") == current_pic['name'])//myPaints[1]["name"].toString())
          {
          synth.cancel();
          //var utterThis = new SpeechSynthesisUtterance();
          //utterThis.stop;
          ...
          if(current_pic["subjects"][37]["check"]==0)//myMsg['shephard2']['check'] == 0)
            {
              utterThis.text = touchs+""+ myMsg['shephard2'][getRandomArbitrary(0,3)];
              current_pic["subjects"][37]["check"] = 1; //Date.now();
              writeSubject(current_pic["subjects"][37]["desc"][0]);//myMsg['shephard2'][1]);
            }
          else
          {
            utterThis.text = /*myMsg['re'][getRandomArbitrary(0,3)]+""+*/touchs+""+current_pic["subjects"][37]["desc"][getRandomArbitrary2(-1,2)];//myMsg['shephard2'][getRandomArbitrary(0,3)];
            writeSubject(current_pic["subjects"][37]["desc"][0]);//myMsg['shephard2'][1]);
            current_pic["subjects"][37]["check"] = 0;//myMsg['shephard2']['check'] = 0;
          }
